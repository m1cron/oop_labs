...
cl_base* cl_base::get_path(string obj) {
    if(obj[1] == '/') return find(obj.substr(2));
    unsigned long long int i = obj.find('/', 1);
    string name = obj.substr(1, i - 1);
    if(this->name == name) return (i == string::npos ? this : this->get_path(obj.substr(i)));
    for(auto o_obj : childs)
        if(o_obj->name == name) return (i == string::npos ? o_obj : o_obj->get_path(obj.substr(i)));
    return nullptr;
}
